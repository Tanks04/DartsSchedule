...
function EventsDialog({open,onClose,events,platformAdmin,organizationAdmin,organizationId,psgzOrganizationId,onSaved,onNotice,t,locale}:{open:boolean;onClose:()=>void;events:CalendarEvent[];platformAdmin:boolean;organizationAdmin:boolean;organizationId:string;psgzOrganizationId:string;onSaved:()=>Promise<void>;onNotice:(value:string)=>void;t:Translator;locale:string}){
  ...
  const compareFields=["eventDate","endDate","startTime","title","discipline","category","venueName","address","city","sourceUrl","note"] as const;
  const optionalOfficialFields=new Set(["endDate","startTime","venueName","address","city","note"]);
  const normalizedComparison=(field:typeof compareFields[number],value:string)=>field==="sourceUrl"?String(value??"").trim().replace(/\/$/,"").toLowerCase():normalizeHeader(value);
  const changes=(snapshot?.events??[]).map(event=>{const existing=events.find(item=>item.organizer===event.organizer&&item.externalId===event.externalId);const changedFields=existing?compareFields.filter(field=>!(optionalOfficialFields.has(field)&&!String(event[field]??"").trim())&&normalizedComparison(field,existing[field])!==normalizedComparison(field,event[field])):[];return{event,existing,changedFields,status:!existing?"new":changedFields.length?"changed":"same"};});
