...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    ...
    const clubs: Club[]=(clubsR.data??[]).map(c=>({id:c.id,organizationId:c.organization_id,name:c.name,contact:c.contact_name,phone:c.phone,email:c.email,note:c.note,venueIds:clubVenueRows.filter(x=>x.club_id===c.id).map(x=>x.venue_id)}));
    const clubNames=new Map(clubs.map(c=>[c.id,c.name]));
    const memberships=membershipR.data??[];
    const teams:Team[]=(teamsR.data??[]).map(team=>({id:team.id,organizationId:team.organization_id,name:team.name,clubId:team.club_id??"",club:clubNames.get(team.club_id)??team.club_name,defaultVenueId:team.default_venue_id??"",defaultVenue:venueNames.get(team.default_venue_id)??"",contact:team.contact_name,phone:team.phone,email:team.email,note:team.note,competitionIds:memberships.filter(m=>m.team_id===team.id).map(m=>m.competition_id)}));
  ...
  const syncAuth = useCallback(async (session:SessionLike, orgId:string) => {
    ...
    if(roleR.error){setRole("viewer");setNotice("Prava nisu dostupna. Pokrenite upgrade_v7_organizations.sql.");return;}
  ...
  useEffect(()=>{
    ...
    if(!teamId||!orgData.teams.some(team=>team.id===teamId))setTeamId(orgData.teams.find(team=>team.id===storedTeam)?.id??orgData.teams.find(team=>team.competitionIds.includes(storedCompetition))?.id??orgData.teams[0]?.id??"");
  ...
  },[orgData,competitionId,teamId,venueId]);

  const leagueTeams=useMemo(()=>orgData.teams.filter(team=>team.competitionIds.includes(competitionId)),[orgData.teams,competitionId]);
  ...
  const watchedTeams=useMemo(()=>{
    if(managedClubIds.length)return orgData.teams.filter(team=>managedClubIds.includes(team.clubId));
    if(editableTeams.length)return orgData.teams.filter(team=>editableTeams.includes(team.id));
    if(organizationAdmin&&selectedTeam?.clubId)return orgData.teams.filter(team=>team.clubId===selectedTeam.clubId);
    return selectedTeam?[selectedTeam]:[];
  },[orgData.teams,managedClubIds,editableTeams,organizationAdmin,selectedTeam]);
  const weeklyRows=watchedTeams.map(team=>({team,match:orgData.matches.filter(m=>(m.homeId===team.id||m.awayId===team.id)&&m.date>=today&&m.date<=weekEnd).sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time))[0]}));
  ...
  function selectCompetition(id:string){setCompetitionId(id);localStorage.setItem(prefLeague,id);const first=orgData.teams.find(team=>team.competitionIds.includes(id));if(first){setTeamId(first.id);localStorage.setItem(prefTeam,first.id);}}
  ...
  return <main className="app-shell">
    ...
    <PermissionsDialog open={permissionsOpen} onClose={()=>setPermissionsOpen(false)} organizationId={organizationId} teams={orgData.teams} clubs={orgData.clubs} onNotice={setNotice} t={t}/>
...
function EditTeamDialog({team,venues,clubs,admin,onClose,onSaved,onNotice,t}:{team:Team|null;venues:Venue[];clubs:Club[];admin:boolean;onClose:()=>void;onSaved:()=>void;onNotice:(s:string)=>void;t:Translator}){
